...
Span* CentralCache::GetOneSpan(SpanList* spanlist, size_t bytes)
{
	...
	{
		if (span->_objlist != nullptr) // _objlist是什么？
		{
			return span;
		}
		span = span->_next;
	...
	//需要进行计算要获取几页npage
	size_t npage = ClassSize::NumMovePage(bytes); //按需计算bytes所需要的页数，最少是1页，bytes越大，页越多
	...
	//这里的end是对于当前内存的最后一个地址的下一个地址。。？不对吧
	// 这是当前Span的末尾地址
	char* end = start + (newSpan->_npage << PAGE_SHIFT);

	char* next = start + bytes;
	char* cur = start;
	// 这是要把他们串起来？因为bytes可能不是正好等于span的大小的，所以会造成内碎片吧，我猜是因为这个
	// 
	while (next < end)
	{
		NEXT_OBJ(cur) = next;
...
// 形参start和end设置为void*&的形式，是为了在函数里面给它们赋值的
size_t CentralCache::FetchRangeObj(void*& start, void*& end, size_t num, size_t bytes)
{
	...
	start = span->_objlist;
	//对于end是内存的最后一块，但是对于最后一块的地址存放在最后的一个内存块的前4个或者8个字节  ？？啥玩意？
	end = prev;
	...
	//将剩下来的内存块再次链接span上
	span->_objlist = cur; // 有可能此时cur已经是nullptr了，自由链表中剩下的，它的头部已经往后挪了

	//给ThreadCache多少个内存块，这个span会使用计数，记录下来
	span->_usecount += fetchnum;

	//进行改进，每次将span中的内存块拿出来的时候，判断这个span中还有没有没申请的内存块，没有就放到最后，
...
{
	//首先求出这个span是在哪一个CentralCache数组的元素上
