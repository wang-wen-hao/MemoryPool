...
{
	return *((void**)obj); // 这和直接void*有什么区别？
}
...
struct Span
{
	...
	Span* _next = nullptr;
	Span* _prev = nullptr;
	// freelist把一个Span分割成一串了，是连续的
	void* _objlist = nullptr; //对象自由链表，应该是改span所属的spanlist?还是freelist? 应该是freelist
...
public:
	...
	static inline size_t Index(size_t bytes)
	{
		...
		//[1, 128]						8byte对齐		freelist[0,16)
		//[129, 1024]					16byte对齐		freelist[17, 72)
		//[1025, 8 * 1024]				128byte对齐		freelist[72, 128) //这里写错了，是128字节对齐。。。
		//[8 * 1024 + 1, 64 * 1024]		512byte对齐		freelist[128, 240)
		if (bytes <= 128)
		{
			return _Index(bytes, 3);
		...
		else if (bytes <= 8 * 1024) //(8 * 8 * 128)
		{
			return _Index(bytes - 1024, 7) + group_array[1] + group_array[0];
		}
		else if (bytes <= 64 * 1024)
		{
			return _Index(bytes - 8 * 1024, 9) + group_array[2] + group_array[1] + group_array[0];
		}
		else
		{
			return -1;
	...
	//计算要获取几页，最少也能获取1页，size越大，获取的页数越多，越到后面直接按照两倍进行补给
